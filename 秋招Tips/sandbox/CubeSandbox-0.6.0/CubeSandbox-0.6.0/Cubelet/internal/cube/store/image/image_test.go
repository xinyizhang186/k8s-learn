...
func TestImageStore(t *testing.T) {
	...
	for desc, test := range map[string]struct {
		...
		t.Run(desc, func(t *testing.T) {
			s, err := NewFakeStore(ctx, []Image{image})
			assert.NoError(err)
			assert.NoError(s.update(ctx, test.ref, test.image))

			ims, err := s.List(ctx)
			assert.NoError(err)
			assert.Len(ims, len(test.expected))
			for _, expect := range test.expected {
				got, err := s.Get(ctx, expect.ID)
				assert.NoError(err)
				equal(got, expect)
				for _, ref := range expect.References {
					id, err := s.Resolve(ctx, ref)
					assert.NoError(err)
