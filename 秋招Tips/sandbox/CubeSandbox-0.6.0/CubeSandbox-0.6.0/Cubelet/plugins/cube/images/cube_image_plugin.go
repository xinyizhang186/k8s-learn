...
func init() {
	...
	registry.Register(&plugin.Registration{
		...
		InitFn: func(ic *plugin.InitContext) (interface{}, error) {
			...
			defaultSnapshotter := config.Snapshotter
			for name, s := range allSnapshotters {
				options.Snapshotters[name] = s
			}
			...
			metaDB, err := ic.GetByID(constants.CubeMetaStorePlugin, constants.MultiMetaID.ID())
			if err != nil {
				return nil, fmt.Errorf("failed to get meta db: %w", err)
			}
			options.MetaDB = metaDB.(multimeta.MetadataDBAPI)
