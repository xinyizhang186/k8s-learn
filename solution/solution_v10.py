"""
HiF4 solution.py — NVFP4 → HiF4 量化转换 (V10: GQA分组稳健选择)
...
核心优化:
  ...
  8. V10 GQA分组选择: sampled-Q/full-KV评估 + 双向cross-fit + per-KV-group配置
...
# ======================================================================
# V10: full-K/V, per-KV-group robust Attention selection
# ----------------------------------------------------------------------
# Two structural fixes over V9:
#   1) calibration samples only Q rows; K/V keep the full sequence so the
#      softmax denominator matches deployment;
#   2) GQA is separable by KV group at the output, so each KV group may keep
#      the globally selected configuration or switch to a different *validated*
#      configuration.  A reverse 3+2 cross-fit must confirm every refinement.
# No new quantization family is introduced here; V10 only selects among the
# already validated V9 candidates at the mathematically correct granularity.
# ======================================================================

ATTN_V10_GROUP_MARGIN = 0.99      # candidate must beat base by >=1% on average
ATTN_V10_GROUP_WORST = 0.999      # and must not lose on any held-out sample
ATTN_V10_SAFE_MEAN = 0.97         # candidate itself must beat Standard robustly
ATTN_V10_SAFE_WORST = 0.999


def _attention_group_mse(out, ref, q_num_heads, kv_num_heads, head_dim):
    """Return one MSE per GQA KV group.

    Each KV group owns ``group=q_num_heads//kv_num_heads`` Q heads and one K/V
    head.  The final Attention output is a concatenation of Q-head outputs, so
    squared error is exactly additive across these groups.
    """
    nq = int(out.shape[0])
    group = q_num_heads // kv_num_heads
    err = (out - ref).reshape(nq, q_num_heads, head_dim).pow(2)
    err = err.reshape(nq, kv_num_heads, group, head_dim)
    return err.mean(dim=(0, 2, 3)).float().contiguous()


def _sample_q_rows_only(q, max_queries=ATTN_EVAL_TOKENS):
    seq = int(q.shape[0])
    if seq <= max_queries:
        return q
    idx = torch.linspace(0, seq - 1, steps=max_queries).round().long().unique()
    return q[idx]


def _evaluate_attention_configs_groupwise(calib_qkv_list, qspecs,
                                            q_num_heads, kv_num_heads, head_dim):
    """Evaluate V9 candidates with sampled Q against *full* K/V.

    Returns ``losses[name][sample] -> Tensor[kv_num_heads]``.  This preserves
    the real softmax key distribution while exposing the exact per-group loss
    decomposition needed for GQA-aware selection.
    """
    losses = {"std": [], "std_vopt": []}
    for spec in qspecs:
        losses[spec["name"] + "_vstd"] = []
    ...
    for sample in calib_qkv_list:
        q_full = _dequant_nvfp4(*sample["q"]).float()
        k_fp = _dequant_nvfp4(*sample["k"]).float()
        v_fp = _dequant_nvfp4(*sample["v"]).float()
        qs = _sample_q_rows_only(q_full)

        ref = _attention_rect(qs, k_fp, v_fp,
                              q_num_heads, kv_num_heads, head_dim)

        q_std = _hif4_dequant(standard_hif4_quantize(qs), qs.shape)
        k_std = _hif4_dequant(standard_hif4_quantize(k_fp), k_fp.shape)
        v_std = _hif4_dequant(standard_hif4_quantize(v_fp), v_fp.shape)
        out_std = _attention_rect(q_std, k_std, v_std,
                                  q_num_heads, kv_num_heads, head_dim)
        losses["std"].append(_attention_group_mse(
            out_std, ref, q_num_heads, kv_num_heads, head_dim
        ))

        nv = _attention_candidate_count(v_fp)
        v_opt = _hif4_dequant(
            _quantize_hif4(v_fp, n_candidates=nv), v_fp.shape
        )
        out_std_vopt = _attention_rect(q_std, k_std, v_opt,
                                       q_num_heads, kv_num_heads, head_dim)
        losses["std_vopt"].append(_attention_group_mse(
            out_std_vopt, ref, q_num_heads, kv_num_heads, head_dim
        ))

        for spec in qspecs:
            q_work, k_work = _apply_attention_transform(
                qs, k_fp, q_num_heads, kv_num_heads, head_dim,
                spec["balance"], spec["hadamard"]
            )
            nq = _attention_candidate_count(q_work)
            nk = _attention_candidate_count(k_work)
            q_hat = _hif4_dequant(
                _quantize_hif4(q_work, n_candidates=nq,
                               importance=spec["q_imp"]),
                q_work.shape,
            )
            k_hat = _hif4_dequant(
                _quantize_hif4(k_work, n_candidates=nk,
                               importance=spec["k_imp"]),
                k_work.shape,
            )
            out_vstd = _attention_rect(q_hat, k_hat, v_std,
                                       q_num_heads, kv_num_heads, head_dim)
            out_vopt = _attention_rect(q_hat, k_hat, v_opt,
                                       q_num_heads, kv_num_heads, head_dim)
            losses[spec["name"] + "_vstd"].append(_attention_group_mse(
                out_vstd, ref, q_num_heads, kv_num_heads, head_dim
            ))
            losses[spec["name"] + "_vopt"].append(_attention_group_mse(
                out_vopt, ref, q_num_heads, kv_num_heads, head_dim
            ))

    return losses


def _aggregate_group_losses(group_losses):
    """Convert per-group losses to the scalar loss format used by V9 selector."""
    return {
        name: [float(x.mean().item()) for x in vals]
        for name, vals in group_losses.items()
    }


def _group_ratios(losses, name, g, denom_name="std"):
    vals = losses.get(name, [])
    den = losses.get(denom_name, [])
    if not vals or len(vals) != len(den):
        return None
    out = []
    for a, b in zip(vals, den):
        bv = float(b[g].item())
        out.append(float(a[g].item()) / max(bv, 1e-15))
    return out


def _config_is_safe_crossfit(loss1, loss2, name, g):
    """Require a candidate to beat Standard on both disjoint held-out splits."""
    r1 = _group_ratios(loss1, name, g, "std")
    r2 = _group_ratios(loss2, name, g, "std")
    ...
    ok = (all(r < 1.0 for r in ratios)
          and mean_r <= ATTN_V10_SAFE_MEAN
          and worst_r <= ATTN_V10_SAFE_WORST)
    return ok, mean_r, worst_r


def _config_beats_base_crossfit(loss1, loss2, cand, base, g):
    """Check that a refinement improves the current V9 base on both splits."""
    r1 = _group_ratios(loss1, cand, g, base)
    r2 = _group_ratios(loss2, cand, g, base)
    if r1 is None or r2 is None:
        return False, float("inf")
    ratios = r1 + r2
    mean_r = sum(ratios) / len(ratios)
    ok = (all(r <= ATTN_V10_GROUP_WORST for r in ratios)
          and mean_r <= ATTN_V10_GROUP_MARGIN)
    return ok, mean_r


def _best_safe_group_candidate(losses, g):
    """Best candidate on one held-out split under the same conservative gate."""
    best = "std"
    best_mean = 1.0
    for name in losses:
        if name == "std":
            continue
        rs = _group_ratios(losses, name, g, "std")
        if not rs:
            continue
        ...
        if (all(r < 1.0 for r in rs)
                and mean_r <= 0.96
                and worst_r <= 0.995
                and mean_r < best_mean):
            best = name
            best_mean = mean_r
    return best


def _select_group_refinements(loss1, loss2, base_name, kv_num_heads):
    """Refine a global V9 mode independently for each GQA KV group.

    To avoid turning head-wise selection into a high-variance search, V10 uses
    a *consensus* rule: the same configuration must be the best safe candidate
    on both disjoint held-out directions.  It must then also beat the deployed
    global base on every held-out sample.
    """
    common = set(loss1.keys()) & set(loss2.keys())
    configs = [base_name for _ in range(kv_num_heads)]

    for g in range(kv_num_heads):
        b1 = _best_safe_group_candidate(loss1, g)
        b2 = _best_safe_group_candidate(loss2, g)
        if b1 != b2 or b1 not in common or b1 == base_name:
            continue
        cand = b1
        if cand != "std":
            safe, _, _ = _config_is_safe_crossfit(loss1, loss2, cand, g)
            if not safe:
                continue
        better, _ = _config_beats_base_crossfit(
            loss1, loss2, cand, base_name, g
        )
        if better:
            configs[g] = cand
    return configs
...
def _mixture_beats_base_crossfit(loss1, loss2, configs, base_name):
    """Require the complete mixed configuration to beat the global base clearly.

    This final aggregate gate prevents many tiny head-wise wins from creating a
    high-variance mixed policy.  Every held-out sample in both directions must
    improve by at least 1.5%.
    """
    ratios = []
    for losses in (loss1, loss2):
        base_vals = losses.get(base_name, [])
        if not base_vals:
            return False
        for i, base_vec in enumerate(base_vals):
            num = 0.0
            for g, cfg in enumerate(configs):
                vals = losses.get(cfg, [])
                if len(vals) != len(base_vals):
                    return False
                num += float(vals[i][g].item())
            mix = num / max(len(configs), 1)
            base = float(base_vec.mean().item())
            ratios.append(mix / max(base, 1e-15))
    return bool(ratios) and all(r <= 0.985 for r in ratios)

def _evaluate_direct_vopt_groupwise(calib_qkv_list, q_num_heads,
                                     kv_num_heads, head_dim):
    """All-calibration, data-independent Vopt check, one KV head at a time."""
    ratios = [[] for _ in range(kv_num_heads)]
    for sample in calib_qkv_list:
        q = _dequant_nvfp4(*sample["q"]).float()
        k = _dequant_nvfp4(*sample["k"]).float()
        v = _dequant_nvfp4(*sample["v"]).float()
        qs = _sample_q_rows_only(q)
        ref = _attention_rect(qs, k, v, q_num_heads, kv_num_heads, head_dim)
        ...
        v_std = _hif4_dequant(standard_hif4_quantize(v), v.shape)
        v_opt = _hif4_dequant(
            _quantize_hif4(v, n_candidates=_attention_candidate_count(v)), v.shape
        )
        out_std = _attention_rect(q_std, k_std, v_std,
                                  q_num_heads, kv_num_heads, head_dim)
        out_opt = _attention_rect(q_std, k_std, v_opt,
                                  q_num_heads, kv_num_heads, head_dim)
        m0 = _attention_group_mse(out_std, ref, q_num_heads, kv_num_heads, head_dim)
        m1 = _attention_group_mse(out_opt, ref, q_num_heads, kv_num_heads, head_dim)
        for g in range(kv_num_heads):
            ratios[g].append(float(m1[g].item()) / max(float(m0[g].item()), 1e-15))
    return ratios


def _parse_attention_config(name):
    if name == "std":
        return "std", False
    if name == "std_vopt":
        return "std", True
    if name.endswith("_vstd"):
        return name[:-5], False
    if name.endswith("_vopt"):
        return name[:-5], True
    return name, False


def _build_groupwise_attention_states(configs, qspecs,
                                       q_num_heads, kv_num_heads, head_dim,
                                       balance_fit, H):
    group = q_num_heads // kv_num_heads
    spec_map = {s["name"]: s for s in qspecs}
    q_modes, k_modes, v_opt_groups = [], [], []
    q_use_imp, k_use_imp = [], []
    q_imp = torch.ones(q_num_heads * head_dim, dtype=torch.float32)
    k_imp = torch.ones(kv_num_heads * head_dim, dtype=torch.float32)

    for g, cfg in enumerate(configs):
        qname, vopt = _parse_attention_config(cfg)
        v_opt_groups.append(bool(vopt))
        if qname == "std":
            q_modes.append("std")
            k_modes.append("std")
            q_use_imp.append(False)
            k_use_imp.append(False)
            continue

        spec = spec_map.get(qname)
        if spec is None:
            q_modes.append("std")
            k_modes.append("std")
            q_use_imp.append(False)
        ...
        q_modes.append(base)
        k_modes.append(base)
        has_qimp = spec["q_imp"] is not None
        has_kimp = spec["k_imp"] is not None
        q_use_imp.append(bool(has_qimp))
        k_use_imp.append(bool(has_kimp))

        if has_qimp:
            s = g * group * head_dim
            e = (g + 1) * group * head_dim
            q_imp[s:e] = spec["q_imp"][s:e].float()
        if has_kimp:
            s = g * head_dim
            e = (g + 1) * head_dim
            k_imp[s:e] = spec["k_imp"][s:e].float()

    common = {
        "groupwise": True,
        "group": int(group),
        "head_dim": int(head_dim),
        "balance": None if balance_fit is None else balance_fit.float().contiguous(),
        "hadamard": H,
    }
    q_state = dict(common)
    q_state.update({
        "group_modes": q_modes,
        "use_imp_groups": q_use_imp,
        "importance": q_imp.contiguous(),
        "mode": "v10_groupwise",
    })
    k_state = dict(common)
    k_state.update({
        "group_modes": k_modes,
        "use_imp_groups": k_use_imp,
        "importance": k_imp.contiguous(),
        "mode": "v10_groupwise",
    })
    v_state = {
        "groupwise": True,
        "use_opt_groups": v_opt_groups,
        "head_dim": int(head_dim),
        "mode": "v10_groupwise",
    }
...
def _concat_hif4_parts(parts):
    if not parts:
        raise ValueError("empty HiF4 part list")
    return {k: torch.cat([p[k] for p in parts], dim=1).contiguous()
            for k in parts[0]}


def _quantize_q_groupwise(q_fp, q_num_heads, head_dim, state):
    modes = list(state["group_modes"])
    kv_num_heads = len(modes)
    group = q_num_heads // kv_num_heads
    balance = state.get("balance")
    H = state.get("hadamard")
    imp_all = state.get("importance")
    use_imp = list(state.get("use_imp_groups", [False] * kv_num_heads))
    parts = []
    for g, mode in enumerate(modes):
        c0 = g * group * head_dim
        c1 = (g + 1) * group * head_dim
        x = q_fp[:, c0:c1]
        if mode == "std":
            parts.append(standard_hif4_quantize(x))
            continue
        if mode in ("bal", "balhad") and balance is not None:
            d = balance[g].to(torch.float32)
            x = (x.reshape(x.shape[0], group, head_dim) * d.view(1, 1, -1)).reshape_as(x)
        if mode in ("had", "balhad") and H is not None:
            x = _apply_hadamard(x, H.to(torch.float32))
        imp = None
        if use_imp[g] and imp_all is not None:
            imp = imp_all[c0:c1]
        parts.append(_quantize_hif4(
            x, n_candidates=_attention_candidate_count(x), importance=imp
        ))
    return _concat_hif4_parts(parts)


def _quantize_k_groupwise(k_fp, kv_num_heads, head_dim, state):
    modes = list(state["group_modes"])
    balance = state.get("balance")
    H = state.get("hadamard")
    imp_all = state.get("importance")
    ...
    for g, mode in enumerate(modes):
        ...
        if mode == "std":
            ...
            continue
        if mode in ("bal", "balhad") and balance is not None:
            x = x / balance[g].to(torch.float32).clamp(min=1e-8).view(1, -1)
        if mode in ("had", "balhad") and H is not None:
            x = _apply_hadamard(x, H.to(torch.float32))
        imp = None
        if use_imp[g] and imp_all is not None:
...
def _quantize_v_groupwise(v_fp, kv_num_heads, head_dim, state):
    use_opt = list(state["use_opt_groups"])
    parts = []
    for g in range(kv_num_heads):
        c0 = g * head_dim
        c1 = (g + 1) * head_dim
        ...
        if use_opt[g]:
            parts.append(_quantize_hif4(
                x, n_candidates=_attention_candidate_count(x)
            ))
        else:
            parts.append(standard_hif4_quantize(x))
    return _concat_hif4_parts(parts)
...
def hif4_calibration_attention(
    ...
    """V10 robust Attention calibration.

    The base mode is selected with sampled-Q/full-KV final-output MSE.  When
    five calibration samples are available, an independent reverse 3+2 split
    is used only to validate per-KV-group refinements.  The deployed fitted
    tensors remain those from the primary split, preserving deployment match.
    """
    if q_num_heads % kv_num_heads != 0:
    ...
    if eval_calib:
        gl1 = _evaluate_attention_configs_groupwise(
            eval_calib, qspecs, q_num_heads, kv_num_heads, head_dim
        )
        selected = _select_attention_config(_aggregate_group_losses(gl1))
    else:
        gl1 = None
        selected = "std"

    # Start from the V10 full-K/V global selection, then refine only when a
    # reverse cross-fit confirms a group-specific alternative.
    configs = [selected for _ in range(kv_num_heads)]
    if (len(calib_qkv_list) >= 5 and gl1 is not None and head_dim % BLK_SIZE == 0):
        fit2 = calib_qkv_list[2:]
        eval2 = calib_qkv_list[:2]
        bal2, stab2 = _compute_qk_balance(
            fit2, q_num_heads, kv_num_heads, head_dim
        )
        ok2 = bal2 is not None and stab2 <= ATTN_BALANCE_STABILITY_MAX
        qspecs2 = _build_attention_qspecs(
            fit2, q_num_heads, kv_num_heads, head_dim, bal2, ok2, H
        )
        gl2 = _evaluate_attention_configs_groupwise(
            eval2, qspecs2, q_num_heads, kv_num_heads, head_dim
        )
        proposed = _select_group_refinements(
            gl1, gl2, selected, kv_num_heads
        )
        if _mixture_beats_base_crossfit(gl1, gl2, proposed, selected):
            configs = proposed

    # V9's successful direct V optimization is data-independent.  If the base
    # Q/K configuration is Standard for a group, validate Vopt on all five
    # calibration samples for that group rather than forcing all KV heads to
    # switch together.
    if calib_qkv_list and head_dim == BLK_SIZE:
        direct_ratios = _evaluate_direct_vopt_groupwise(
            calib_qkv_list, q_num_heads, kv_num_heads, head_dim
        )
        for g in range(kv_num_heads):
            qname, _ = _parse_attention_config(configs[g])
            if qname != "std":
                continue
            rs = direct_ratios[g]
            if rs and all(r < 0.999 for r in rs):
                configs[g] = "std_vopt"

    # If every group converges to one configuration, use the legacy compact
    # path.  Otherwise deploy the genuine per-group state.
    if configs and any(c != configs[0] for c in configs):
        return _build_groupwise_attention_states(
            configs, qspecs, q_num_heads, kv_num_heads, head_dim,
            balance_fit, H
        )
    if configs:
        selected = configs[0]

    # Keep V9's calibrated rescue machinery for pure-standard cases where the
    # per-group selector has no evidence to change anything.
    if selected == "std" and calib_qkv_list and head_dim == BLK_SIZE:
        vprof = _v_only_profile(
            calib_qkv_list, q_num_heads, kv_num_heads, head_dim
        )
        if vprof is not None and vprof["is_v_only"]:
            factor, _ = _fit_v_attention_factor(
                fit_calib, q_num_heads, kv_num_heads, head_dim
            )
...
def hif4_dynamic_quantize_q(
    ...
    if isinstance(q_state, dict) and q_state.get("groupwise", False):
        return _quantize_q_groupwise(q_fp, q_num_heads, head_dim, q_state)
...
def hif4_dynamic_quantize_k(
    ...
    if isinstance(k_state, dict) and k_state.get("groupwise", False):
        return _quantize_k_groupwise(k_fp, kv_num_heads, head_dim, k_state)
...
def hif4_dynamic_quantize_v(
    ...
    if isinstance(v_state, dict) and v_state.get("groupwise", False):
        return _quantize_v_groupwise(v_fp, kv_num_heads, head_dim, v_state)
