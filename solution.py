...
def hif4_calibration_and_quantize_weight(
    ...
    w_std_params = _quantize_hif4_impl(weight_fp, n_candidates=n_final, importance=None, cov=None)
    w_std_hat = _hif4_dequant(w_std_params, weight_fp.shape)
    ...
    for act in calib_acts:
        if use_cov:
            act_x = _apply_hadamard(act * (1.0 / D), H)
        else:
            act_x = _apply_hadamard(act, H)
        player_mse += ((act_x @ (w_hat - w_rot).T) ** 2).sum().item()
        std_mse += ((act @ (w_std_hat - weight_fp).T) ** 2).sum().item()
        total_tokens += act.shape[0]
    player_mse /= total_tokens
    std_mse /= total_tokens

    if std_mse <= player_mse:
        weight_params = w_std_params
        w_hat = w_std_hat
        w_rot = weight_fp
        D = torch.ones(K, dtype=torch.float32)
        best_alpha = None
    ...
    w_diag = (w_hat ** 2).sum(dim=0).clamp(min=1e-8)
    C_WW = _compute_ww_block_cov(w_hat, K) if (use_cov and weight_fp.shape[0] <= 1024) else None
