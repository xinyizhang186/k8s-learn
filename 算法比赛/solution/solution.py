...
def _compute_v_importance(calib_qkv_list, q_num_heads, kv_num_heads, head_dim):
    """V token-level importance from P^T P × attention output norm (BoA Hessian).

    ||P E_V||^2 = tr(E_V^T P^T P E_V) ≈ sum_t rho_t * ||E_V[t,:]||^2
    where rho_t = sum_i P[i,t]^2 (column norm squared of attention matrix).

    BoA (arXiv:2406.13474) proves exact H_V = A^TA ⊗ Wout^T·Wout.
    We approximate the Wout^T·Wout factor via attention output per-token norm:
      out_norm_t = ||sum_h P[:,t] · V[t,:]||^2 (from calibration Q,K,V)

    Final: rho_t *= (1 + alpha * out_norm_t_normalized), alpha=0.5.
    ...
    for sample in calib_qkv_list:
        ...
        if rho is None:
            ...
            # Skip Wout computation for large seq (O(seq^2) cost)
            compute_wout = cur_seq <= 256
            if compute_wout:
                out_norm = torch.zeros(kv_num_heads, cur_seq, dtype=torch.float32)
        ...
        for g in range(kv_num_heads):
            ...
            if out_norm is not None:
                ...
                v_g = v_re[g:g + 1]
                out = torch.matmul(attn, v_g.expand(group, -1, -1))
                out_norm[g] += (out ** 2).sum(dim=(0, 2))
    ...
    if out_norm is not None and n_samples > 0:
        out_norm = (out_norm / max(n_samples, 1)).clamp(min=1e-8)
        out_norm_n = out_norm / (out_norm.max() + 1e-12)
        rho = rho * (1.0 + alpha * out_norm_n)
